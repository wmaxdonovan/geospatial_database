...
def delete_governs(database):
    delete_on = input("Deleting from governs. Specify deletion attribute: \n\t"
                      "Delete table: t\n\t"
                      "Delete on land_id: l\n\t"
                      "Delete on county_id: o\n\t").lower()
    if delete_on == 'q':
        util.exit_TEAL(database)
    elif delete_on == 't':
        ...
        if delete_id.isdigit():
            database.delete('governs', 'land_id', '=' + delete_id)
        else:
            sys.stdout.write('id must be an integer\t')
            delete_land(database)
    elif delete_on == 'o':
        delete_id = input('county id to delete: \t')
        if delete_id.isdigit():
            database.delete('governs', 'owner_id', '=' + delete_id)
        else:
            sys.stdout.write('id must be an integer\t')
            delete_land(database)
    else:
